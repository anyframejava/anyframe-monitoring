...
public class InfraREDServletFilter implements Filter {
	private static final Logger log = LoggingFactory
			.getLogger(InfraREDServletFilter.class);
		...
		try {
			if (request instanceof HttpServletRequest) {
				...
				if (log.isDebugEnabled()) {
					log
							.debug("This is an HTTP request InfraREDServletFilter, so capturing the URI "
									+ uri);
				}
				...
				log
						.debug("This is not an HTTP request InfraREDServletFilter, so ignoring");
				chain.doFilter(request, response);
			}
			...
			log.info("ServletException Occurred : "
					+ servletException.getMessage());
			throw servletException;
		} catch (Throwable t) {
			log.info("Throwable Occurred : " + t.getMessage());
			throw new ServletException("unknown exception occurred.", t);
		} finally {
			ThreadLocal treadLocal = facade.getThreadLocal();
			treadLocal.remove();
			log.debug("facade.getStatisticsThreadLocal() is removed.");
		}
	}
