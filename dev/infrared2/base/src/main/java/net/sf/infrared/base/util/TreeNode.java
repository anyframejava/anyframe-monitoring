...
//        }
        int index=0;
        boolean hasItem = hasItem(index, this.childNodes);
        while (hasItem){
            TreeNode childNode = (TreeNode) this.childNodes.get(index);
            if (merger.isMatching(childNode, node)) {
                nodeToMergeOnto = childNode;
                break;
            }
            ...
            index++;
            hasItem = hasItem(index, this.childNodes);
        }
...
//        }
        ...
        int index2=0;
        boolean hasItem2 = hasItem(index2, node.childNodes);
        while (hasItem2){
            TreeNode child = (TreeNode) node.childNodes.get(index2);
            nodeToMergeOnto.mergeAsChild(child, merger);
            
            index2++;
            hasItem2 = hasItem(index2, node.childNodes);
        }
    }
     ...
     */
    private boolean hasItem(int index, List list) {
