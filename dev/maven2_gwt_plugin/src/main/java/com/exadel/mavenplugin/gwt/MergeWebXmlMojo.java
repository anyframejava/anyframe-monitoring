...
public class MergeWebXmlMojo extends AbstractGWTMojo {
    ...
    public void execute() throws MojoExecutionException, MojoFailureException {
        System.out.println("MergeWebXmlMojo: execute...");
        ...
        if (getProject().getProperties().getProperty("google.webtoolkit.home") == null) {
            ...
            File targetDir = null;
            try {
                targetDir = new File(getGwtBinDirectory(), GWTSetup.guessArtifactId() + "-"
                                + getGwtVersion()).getCanonicalFile();
                getProject().getProperties().setProperty("google.webtoolkit.home",
        ...
        try {
            ...
            for (Object target : this.getCompileTarget().keySet()) {
                File moduleFile = null;

                for (Iterator it = this.getProject().getCompileSourceRoots().iterator(); it
                                .hasNext()
                                && moduleFile == null;) {

                    File check = new File(it.next().toString() + "/" + ((String)target).replace('.', '/')
                                    + ".gwt.xml");
                ...
                for (Iterator it = this.getProject().getResources().iterator(); it.hasNext();) {
                    ...
                    File check = new File(r.getDirectory() + "/" + ((String)target).replace('.', '/')
                                    + ".gwt.xml");
                    System.out.println("Looking for file: " + check.getAbsolutePath());
                    moduleFile = check.exists() ? check : moduleFile;
                }
                ...
                try {
                    if (moduleFile != null) {
                        processor = new GwtWebInfProcessor((String)target, moduleFile, destination
                                        .getAbsolutePath(), destination.getAbsolutePath());
                    } else {
                        System.out.println("WebXML: " + this.getWebXml().getAbsolutePath());
                        processor = new GwtWebInfProcessor((String)target, destination.getAbsolutePath(),
