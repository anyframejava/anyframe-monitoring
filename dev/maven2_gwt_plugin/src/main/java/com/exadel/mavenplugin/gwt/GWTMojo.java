...
public class GWTMojo extends AbstractGWTMojo {
    ...
    public Collection<File> buildRuntimeClasspathList()
        ...
        Collection<File> classpathItems = super.buildRuntimeClasspathList();

        List libs = this.getHostedModeExtendedLibraries();
        if (libs != null && libs.size() > 0) {
            for (Object fileName : libs) {
                File file = new File((String) fileName);
                if (file.exists()) {
                    if (file.isDirectory()) {
                        Collections.addAll(classpathItems, file.listFiles());
                    } else {
                        classpathItems.add(file);
                    }
                }
            }
        }
