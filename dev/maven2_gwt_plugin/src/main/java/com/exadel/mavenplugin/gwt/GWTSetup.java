...
public class GWTSetup extends AbstractGWTMojo {
  ...
  public void execute() throws MojoExecutionException, MojoFailureException {
    ...
    //clean gwt-cache
    File cache = new File(this.getProject().getBasedir(), ".gwt-cache");
    System.out.println("Search for "+cache.getAbsolutePath());
    if (cache.exists()) {
        try {
            System.out.println("Deleting gwt-cache directory");
            FileUtils.deleteDirectory(cache);
        } catch (IOException e) {
            System.out.println("Error can not delete directory: " + cache.getAbsolutePath());
            e.printStackTrace();
        }
    }
  }
