...
<head>
	...
	<script language="JavaScript">
		function fncAddProductView() {
			document.location.href="<c:url value='/monitoringAddViewProduct.do'/>";
		}	
		function fncSearchProduct(arg) {
		   	document.searchForm.action="<c:url value='/monitoringListProduct.do'/>";
...
<display:table name="productList" class="table" id="productList" requestURI="monitoringListProduct.do" export="false" partialList="true" size="${size}" pagesize="${pagesize}" style="margin-top:10px;">
	<display:column sortable="true" media="html" titleKey="category.categoryNo">
		<a href="<c:url value='/monitoringGetCategory.do?categoryNo=${productList.category.categoryNo}'/>">${productList.category.categoryNo}</a>
	</display:column>
	<display:column property="prodNo" sortable="true" href="${ctx}/monitoringGetProduct.do" media="html" paramId="prodNo" paramProperty="prodNo" titleKey="product.prodNo"/>
    ...
    <displayetProperty name="paging.banner.group_size" value="${pageunit}" />
</display:table>
